# GUI-independent tmux configuration shared between the home-manager module
# (desktops, modules/home-manager/tmux.nix) and the NixOS server profile
# (modules/nixos/server-profile.nix). Theme, keybindings, splits, mouse and
# activity settings — everything that is identical whether the shell is on a
# laptop or a headless box.
#
...
''
  ...
  # Refresh session environment on attach so new panes pick up the live SSH
  # agent (agent-forwarded connections) and, on a desktop, the current
  # compositor (WAYLAND_DISPLAY/SWAYSOCK) — a pre-warmed tmux server is born
  # without them.
