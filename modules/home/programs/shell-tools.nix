...
let
  cfg = config.personal.shell-tools;
in
{
  options.personal.shell-tools = {
    ...
    direnv = {
      ...
      nix-direnv = {
        enable = mkOption {
          type = types.bool;
          default = false;
          description = "Whether to enable nix-direnv for better performance with Nix";
        };
      };
    };
    ...
    simpleTools = {
      enable = mkOption {
        type = types.bool;
        default = true;
        description = "Whether to enable simple shell tools with minimal configuration";
      };
  ...
  config = mkIf cfg.enable {
    programs = {
      ...
      broot.enable = mkIf (cfg.simpleTools.enable && elem "broot" cfg.simpleTools.enabledTools) true;
      command-not-found.enable = mkIf (cfg.simpleTools.enable && elem "command-not-found" cfg.simpleTools.enabledTools) true;
      jq.enable = mkIf (cfg.simpleTools.enable && elem "jq" cfg.simpleTools.enabledTools) true;
    };
  };
}
