...
in
{
  ...
  config = mkIf cfg.enable {
    services.gpg-agent = {
      enable = true;
      pinentry.package = 
        if cfg.pinentryFlavor == "gtk2" then pkgs.pinentry-gtk2
        else if cfg.pinentryFlavor == "gnome3" then pkgs.pinentry-gnome3
        else if cfg.pinentryFlavor == "qt" then pkgs.pinentry-qt
        else if cfg.pinentryFlavor == "curses" then pkgs.pinentry-curses
        else if cfg.pinentryFlavor == "tty" then pkgs.pinentry-tty
        else if cfg.pinentryFlavor == "emacs" then pkgs.pinentry-emacs
        else pkgs.pinentry;
